...
The CLI automatically redacts personally identifiable information in non-interactive mode. This behavior is structural and not controlled by a caller flag.

- In redirected/agent output, PII values appear as deterministic tokens such as `«PII:EMAIL:a3f2b1c9»`.
- Tokens are stable for the same source value on the same machine.
- You may pass tokens back to create/update commands; the CLI resolves them to real values before API calls.
- Non-interactive responses include `piiRedaction` and `piiRedactionReason` so you can verify mode.

Example token round-trip:

```shell
nexudus coworkers update 12345 --email "«PII:EMAIL:a3f2b1c9»" --agent
```

Do not treat tokens as real values and do not attempt to reverse them.
