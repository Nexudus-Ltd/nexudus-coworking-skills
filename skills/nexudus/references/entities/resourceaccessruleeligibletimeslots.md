...
A ResourceAccessRuleEligibleTimeSlot defines a day and time window during which a resource access rule applies. Multiple time slots can be configured to restrict when a resource can be booked under a particular access rule. Time values are stored in UTC. The base date is 1976-01-01 but the actual stored date depends on the time zone offset (e.g., 1975-12-31 for time zones ahead of UTC). Always convert local times to UTC before creating time slots.
